...
const Tables = ({ component: Component, ...rest }) => {
  ...
  const indexOfLastBet = currentPage * todosPerPage;
  const indexOfFirstBet = indexOfLastBet - todosPerPage;
  const currentBet = datas.slice(indexOfFirstBet, indexOfLastBet);

  console.log("currentBetcurrentBet", currentBet);
  ...
  const pageNumbers = [];
  for (let i = 1; i <= Math.ceil(datas.length / todosPerPage); i++) {
    pageNumbers.push(i);
  }
